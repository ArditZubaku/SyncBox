...
public class IconResolver {
  private static final Logger LOGGER = Logger.getLogger(IconResolver.class.getName());
  ...
  public Node getIconForFolder(String folderName) {
    ...
    try (InputStream inputStream =
        getClass().getResourceAsStream(Paths.IconsPackage + iconFileName)) {
      if (inputStream != null) {
        return new ImageView(new Image(inputStream, 25, 25, true, true));
      } else {
        LOGGER.log(Level.SEVERE, "Icon not found for folder: " + folderNameLowerCase);
        return null;
      }
    } catch (Exception e) {
      LOGGER.log(Level.SEVERE, "Error getting icon for folder: " + folderNameLowerCase, e);
      return null;
    }
  }
